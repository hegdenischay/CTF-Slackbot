  ...
  def getChallenges(self):
    with Halo(text='\n Collecting challs') as sp:
      try:
        ...
        raise Exception("No Challenges Found (but login worked!)")
        # sys.exit()
    return True
  ...
  def parseConfig(self, path):
    with Halo(text='\n Loading an existing data') as sp:
      try:
        ...
        raise Exception("Failed to find chall.json")
        # sys.exit()
    ...
    if response.status_code == 200:
      with open(path, 'wb') as f:
        for chunk in response.iter_content(512*1024):
          if chunk:
            f.write(chunk)
def main(user, passwd, url):
  # changed the code slightly so that we can match ArgumentParser
  myargs = [user,passwd,url]
